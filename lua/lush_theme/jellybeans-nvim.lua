...
local theme = lush(function()
    return {
        ...
        ColorColumn {bg = grey_one}, -- used for the columns set with 'colorcolumn'
        ...
        VertSplit {fg = shuttle_grey, bg = background}, -- the column separating vertically split windows
        Folded {fg = grey_chateau, bg = bright_grey}, -- line used for closed folds
        FoldColumn {fg = shuttle_grey, bg = mine_shaft}, -- 'foldcolumn'
        SignColumn {fg = boulder, bg = background}, -- column where |signs| are displayed
        ...
        NormalFloat {bg = grey_one}, -- Normal text in floating windows.
        ...
        LspDiagnosticsDefaultError {fg = raw_sienna.desaturate(27).lighten(-25)}, -- Used as the base highlight group. Other LspDiagnostic highlights link to this by default (except Underline)
        LspDiagnosticsDefaultWarning {fg = old_brick.desaturate(80).lighten(19)}, -- Used as the base highlight group. Other LspDiagnostic highlights link to this by default (except Underline)
        LspDiagnosticsDefaultInformation {
            fg = bayoux_blue.desaturate(30).lighten(8)
        }, -- Used as the base highlight group. Other LspDiagnostic highlights link to this by default (except Underline)
        LspDiagnosticsDefaultHint {fg = highland.desaturate(50).lighten(-26)}, -- Used as the base highlight group. Other LspDiagnostic highlights link to this by default (except Underline)
        ...
        -- LspDiagnosticsUnderlineError {fg = background}, -- Used to underline "Error" diagnostics
        -- LspDiagnosticsUnderlineWarning {fg = background}, -- Used to underline "Warning" diagnostics
        -- LspDiagnosticsUnderlineInformation {fg = background}, -- Used to underline "Information" diagnostics
        ...
        IndentBlanklineChar {fg = gravel},
        IndentBlanklineContextChar {fg = hoki}, GitSignsAdd {fg = green_smoke},
        GitSignsChange {fg = goldenrod, bg = background},
        GitSignsDelete {fg = raw_sienna, background = background}
